...
def main():
    ...
    report_and_notify(merged_df)

    print("✅ Report generation completed")

# === Report Management ===
def report_and_notify(merged_df):

    SLACK_BOT_TOKEN = os.environ.get("SLACK_PAGAMENTI_REPORT_BOT_TOKEN")
    SLACK_CHANNEL_ID = "C081WTLJHB8"
    ...
    try:
        ...
        # build the top5 text and the comment text for the post
        top5_text = "\n".join([
            f"{i+1}. `{row['broker_id']}` - *{row['broker_name']}* posizioni: `{row['total']:,}`"
            for i, row in chart_df.reset_index(drop=True).iterrows()
        ])
        comment_text = (
            "📈 *GPD - Andamento caricamento ACA per Intermediario*\n"
            "--------------------------------------\n"
            f"*🏆 Top 5 Intermediari per numero ACA:*\n{top5_text}"
        )

        # Upload BOTH files in a single API call to create the initial message
        print("Uploading files to create the initial post...")
        initial_upload_response = client.files_upload_v2(
            ...
            file_uploads=[
                {
                    ...
                    "title": "Dati andamento caricamento ACA"
                },
                {
                    "file": chart_path,
                    "filename": os.path.basename(chart_path),
                    "title": "Distribuzione visuale dei top broker"
                }
            ]
        )

        # Extract the timestamp and file URLs from the response
        files = initial_upload_response.get("files", [])
        if not files:
            raise ValueError("File upload failed, no files returned in the response.")

        print("✅ Slack notification sent and updated successfully as a single post!")
